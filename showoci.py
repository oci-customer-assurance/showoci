...
def execute_extract():
    ...
    if cmd.servicefile or cmd.servicescr:
        ...
        ############################################
        if cmd.csv:
            csv.csv_tags_to_cols = cmd.csvtagstocols
...
def set_parser_arguments(argsList=[]):
    ...
    parser.add_argument('-csvtagstocols', action='store_true', default=False, dest='csvtagstocols', help='Convert Tags to Columns in CSV Extract')
